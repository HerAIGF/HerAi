    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      ...
      >
        <h1 className="text-2xl font-bold mb-4">
          Sign up for {gf || 'your girlfriend'}
        </h1>
        ...
        {/* Added optional prompt textarea */}
        <label className="block mb-2">Optional message to your AI girlfriend</label>
        <textarea
          className="w-full border rounded p-2 mb-3"
          value={form.prompt}
          onChange={handleChange('prompt')}
          placeholder="Say hi or leave a note (optional)"
        />
        ...
        <div className="mb-4">
          <label className="flex items-start gap-3">
            <input
              type="checkbox"
              checked={form.adultConsent}
              onChange={handleChange('adultConsent')}
            />
            <span>
              I consent to receive flirtatious/adult content messages from my
              selected AI girlfriend. (Can opt-out later.)
            </span>
          </label>
        </div>
